...
import SpecsTable from './components/SpecsTable.tsx';
...
function App() {
  ...
  // Move getDisplayLength up so it can be used in memos
  ...
  // FIX: Added missing memoized values for filtering logic
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        const formattedData: BellowsPart[] = rawData.map((row: any) => {
          ...
          return {
            part_number: String(normalized.part_number || normalized.partnumber || ''),
            pipe_size: Number(normalized.pipe_size || 0),
            overall_length_oal_in: Number(normalized.overall_length_oal_in || normalized.oal || 0),
            bellows_id_in: Number(normalized.bellows_id_in || 0),
            bellows_od_in: Number(normalized.bellows_od_in || 0),
            live_length_ll_in: Number(normalized.live_length_ll_in || 0),
            axial_movement_in: Number(normalized.axial_movement_in || 0),
            lateral_movement_in: Number(normalized.lateral_movement_in || 0),
            angular_movement_deg: Number(normalized.angular_movement_deg || 0),
            max_allowable_pressure_psig: Number(normalized.max_allowable_pressure_psig || 0),
            axial_spring_rate_lbf_in: String(normalized.axial_spring_rate_lbf_in || ''),
            lateral_spring_rate_lbf_in: String(normalized.lateral_spring_rate_lbf_in || ''),
            angular_spring_rate_ft_lbs_deg: String(normalized.angular_spring_rate_ft_lbs_deg || ''),
            bellows_material: String(normalized.bellows_material || ''),
            bellows_material_grade: String(normalized.bellows_material_grade || ''),
            pressure_psig: String(normalized.pressure_psig || ''),
            temperature_f: String(normalized.temperature_f || ''),
            number_of_cycles: String(normalized.number_of_cycles || ''),
            cycles_format: String(normalized.cycles_format || ''),
            number_of_plys: String(normalized.number_of_plys || ''),
            weld_neck_material: String(normalized.weld_neck_material || ''),
            weld_neck_grade: String(normalized.weld_neck_grade || ''),
          } as BellowsPart;
        });

        setBellowsList(formattedData);
        setImportInfo({ rows: rawData.length, cols: Object.keys(rawData[0] as object).length, source: file.name });
  ...
  // FIX: Added missing event handlers for unit conversions
